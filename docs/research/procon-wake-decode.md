...
> **⚠️ SUPERSEDED — kept for audit trail.** The central premise of this doc (that the Pro Controller's wake mechanism is a BLE advertisement captured on SDR) was empirically falsified on 2026-04-20 by [`procon-trigger-test-results.md`](procon-trigger-test-results.md). An ESP32 BLE scanner running in full-discovery mode captured 8586 BLE adv events from 19 surrounding devices across multiple HOME-press cycles in multiple bonded/unbonded states; **zero** were from the Pro Controller's MAC. macOS bluetoothd independently classified every event from the controller as `< ClassicScan >` (BR/EDR). The `0x4A9A254E`-shaped pattern HelloRayH observed was almost certainly a **classic-BT Channel Access Code** derived from the host's BD_ADDR, not a BLE access address — the two share Bluetooth-spec ancestry but live on different PHYs. SDR can observe both; an ESP32 BLE radio can only observe BLE. The `ADV_DIRECT_IND` parse below was a provisional read of a screenshot, not a ground-truth protocol decode. **The Pro Controller uses classic-BT paging for wake/reconnect, not BLE.** See [`findings.md`](findings.md) for the corrected model.
