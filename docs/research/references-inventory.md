...
1. **Pro Controller patch RAM is dumpable, unencrypted, and disassembled** (`tmp0` + `tmp0.hop`). This is the most plausible attack surface for "modify the wake adv shape from the controller side." Whether the wake-adv-emission code path lives in the patch RAM or in the BCM ROM (which would still be encrypted) is the open question. A focused Hopper session on `tmp0.hop` could answer this in a day.
