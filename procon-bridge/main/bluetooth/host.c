...
#ifdef CONFIG_PROCON_WAKE_BRIDGE
    {
        extern void wake_bridge_init(void);
        wake_bridge_init();
    }
#endif
...
#ifdef CONFIG_PROCON_WAKE_BRIDGE
    /* switch2controller fork: route Switch Pro Controller input to wake_bridge
     * for HOME-press → Switch 2 wake beacon. Skip the wired-adapter pipeline
     * entirely. Other controller types (PS3/Wii/etc) are silently dropped.
     */
    extern void wake_bridge_on_input(uint8_t report_id, const uint8_t *data, uint32_t len);
    if (device->ids.type == BT_SW || device->ids.type == BT_SW2) {
        wake_bridge_on_input(report_id, data, len);
    }
    (void)bt_data; (void)report_type;
    return;
#endif
