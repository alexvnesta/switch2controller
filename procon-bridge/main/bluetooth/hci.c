...
/* --- Public helpers for wake_bridge ---------------------------------------- */

/* Send LE_Set_Adv_Param (type=non-conn, channels 37/38/39, ~50ms interval),
 * LE_Set_Adv_Data with our raw 31-byte payload, then LE_Set_Adv_Enable.
 * Caller is responsible for stopping the burst via bt_hci_cmd_le_adv_disable().
 *
 * Mirrors the burst-fire logic from esp32/src/switch2_wake.c (NimBLE) but uses
 * BlueRetro's bare-HCI plumbing so it coexists with the classic-BT host code.
 */
void bt_hci_cmd_le_set_adv_data_raw(const uint8_t *payload, uint8_t payload_len) {
    /* 1) LE_Set_Adv_Param — non-connectable, all 3 adv channels, ~50ms interval */
    {
        struct bt_hci_cp_le_set_adv_param *p =
            (struct bt_hci_cp_le_set_adv_param *)&bt_hci_pkt_tmp.cp;
        p->min_interval = 0x0050;       /* 50 ms (in 0.625 ms units = 0x50) */
        p->max_interval = 0x0080;       /* 80 ms */
        p->type = 0x03;                 /* ADV_NONCONN_IND */
        p->own_addr_type = 0x00;        /* public */
        memset(&p->direct_addr, 0, sizeof(p->direct_addr));
        p->channel_map = 0x07;          /* all 3 adv channels */
        p->filter_policy = 0x00;
        bt_hci_cmd(BT_HCI_OP_LE_SET_ADV_PARAM, sizeof(*p));
    }
    /* 2) LE_Set_Adv_Data — our 31-byte wake payload */
    {
        struct bt_hci_cp_le_set_adv_data *d =
            (struct bt_hci_cp_le_set_adv_data *)&bt_hci_pkt_tmp.cp;
        memset(d->data, 0, sizeof(d->data));
        d->len = (payload_len > 31) ? 31 : payload_len;
        memcpy(d->data, payload, d->len);
        bt_hci_cmd(BT_HCI_OP_LE_SET_ADV_DATA, sizeof(*d));
    }
    /* 3) LE_Set_Adv_Enable */
    {
        struct bt_hci_cp_le_set_adv_enable *e =
            (struct bt_hci_cp_le_set_adv_enable *)&bt_hci_pkt_tmp.cp;
        e->enable = 0x01;
        bt_hci_cmd(BT_HCI_OP_LE_SET_ADV_ENABLE, sizeof(*e));
    }
}
